...
- `GET/POST /api/labels` — list (with schedules and the categories each is limited to) + create labels. An empty `categoryIds` means **every** category, not none
- `PUT/DELETE /api/labels/[id]` — update/delete labels (ownership check). An absent `categoryIds` preserves the stored restriction rather than clearing it, the same stale-client guard `applicableTo` has. Narrowing either restriction returns **409** `needsConfirmation` naming how many transactions would lose the label; one confirmation covers both, since a save can narrow both at once
