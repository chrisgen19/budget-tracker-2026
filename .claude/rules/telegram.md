...
paths:
  ...
  - src/components/profile/telegram-digest-setting.tsx
...
**The keyboard** (`src/lib/telegram/quick-keyboard.ts`): `/keyboard` pins a `ReplyKeyboardMarkup` above the message box, `/keyboard off` removes it, and `/help` brings it along. Its buttons are the user's **Quick Log tiles**: the first `KEYBOARD_TILE_LIMIT` (6) in `sortOrder`, two to a row, then a fixed `/summary` `/recent` row. Six is the dashboard strip's number for the strip's reason, so reorder on `/quick-log` decides what shows and there is no second setting. It used to be three hardcoded `QUICK_FARES` strings, each complete shorthand the logging path parsed like typed text, which meant a deploy to change a fare and no button for anything without a fixed amount; the tiles already had an editor, a category, pinned labels and a nullable amount, so the keyboard became one more surface over them. A button reads `Office · ₱38`, or `Grab · ₱?` for a tile that asks, and that text is deliberately **not** shorthand (`quick-keyboard.test.ts` asserts `parseShorthandEntries` and `resolveCommand` both return nothing for it): the old approach made the label carry the write, and a tile's category and pins cannot fit in a label. The bot instead recognises the shape with `looksLikeTileButton` (a middle dot and the currency symbol, so ordinary messages never pay for a read), fetches the tiles fresh through the read-only MCP tool `get_quick_tiles`, and matches the text exactly against **all** tiles, not only the six shown. That check runs before `isScanCorrection`, which would otherwise take a button's text as the new description of a waiting receipt. A button that matches nothing was renamed, re-priced or deleted since the keyboard was sent: nothing is logged, and the reply carries a fresh keyboard, which is the only way a reply keyboard ever updates. A fixed tile logs through `create_transactions` with `resolvedCategoryId` and only the pins whose `applies` holds, `labelIds` **absent** when there are none so schedules run; the bot resolves nothing itself, since `get_quick_tiles` returns `viewTiles`, the same read the Mini App and `/quick-log` use. An asking tile stores a `PendingAmount` (`pending-amount.ts`: in memory, one per chat, five minutes) and replies "How much for Grab?"; the next message that is **only** a number (`parseBareAmount`) logs it, and any other message drops the prompt so a figure typed later for another reason is not filed under a forgotten button. "Any other" is enforced by where the check sits: first in `handleMessage`, ahead of the photo and scan-confirmation returns, which originally ran before it and left the prompt armed, plus a clear in `handleCallback` for button presses, which never reach `handleMessage`. The tile is read again at that point, and a deleted tile or one that has since gained a fixed amount is refused rather than guessed at. The idempotency key is the update carrying the write: the tap for a fixed tile, the number for an asking one. A failed tile read still pins the command row and says the buttons could not be loaded, since an empty keyboard would report a network problem as a fact about the account. `get_quick_tiles` needs `budget:read` **and** `labels:read`, both in the default grant, so the bot's token needed no re-mint. With fewer than six saved, the spare slots are filled from **Frequent** (`keyboardButtons`), read through a second read-only tool, `get_frequent_tiles` (`budget:read` **and** `transactions:read`, since it is derived from real rows), which runs the same `loadFrequentTiles` the Mini App grid does with the saved buttons' descriptions excluded. Saved buttons always win a slot, a Frequent button is skipped when its text equals a saved tile's (a tap is matched against saved tiles first and would log the tile instead), and two Frequent buttons never share a text. A Frequent entry shows its amount only when `amountIsStable`, and otherwise `?` and the same how-much prompt: the Mini App's rule that the user may assert a fixed amount and the system may never infer one. It logs with **no** `labelIds`, as a Mini App tap on it does. Its prompt holds a snapshot of description and category rather than an id (`PendingAmountSource`), which is safe where it would not be for a tile: nothing edits a Frequent entry, and its category came off real transactions, which `onDelete: Restrict` keeps from being deleted. Frequent is read only after the saved tiles miss, so a tap on a saved button costs no second call. Entries move as habits change, so a tap on one that has dropped out or changed its usual amount gets the same "changed" reply and a fresh keyboard. A failed Frequent read costs only the fill, so it is logged and not reported in the chat. A failed **saved-tile** read is the opposite case and fills nothing: the text check can only keep a Frequent button off a saved tile's text when it can see the tiles, so without them a Frequent "Coffee" could carry a saved "Coffee" tile's text and, once the read recovers, a tap on it would log the tile. For the same reason a tap never falls through to Frequent matching after the tile read failed, and gets the command-only keyboard instead.
...
**The Watchlist digest** (`src/lib/telegram/watchlist-digest.ts`, pure, and `watchlist-digest-send.ts`, the I/O) rides the same cron, the same owner scoping and the same one-recipient 409, and nothing else: its own switch and time in Profile > Features (`users.telegram_watchlist_digest`, `_time`, default `08:00`), its own claimed day in `telegram_watchlist_digests`, and a failure in either never costs the other its day. At most one message a day, **every** day rather than weekdays, since a card falls due on a Saturday too. It carries only the kinds `DIGEST_KINDS` marks actionable (bills and card payments due or missed, untracked card interest, renewals, expected income missing, cash shortfall, budget limits), an exhaustive `Record` so a new finding kind does not compile until someone decides whether it interrupts the owner's day. It reads the **current calendar month** at monthly granularity, the only period budget findings exist for, and keys findings with the app's own `watchlistFindingKey`, so resolving or snoozing one on the Watchlist silences it here too. What was sent is stored as `telegram_watchlist_alerts` rows holding the SHA-256 of that key, never the text, and is not sent again; a finding whose key moves (a new occurrence, a new budget band) is new. A quiet day keeps its claim, so a finding raised in the afternoon waits for tomorrow rather than becoming a second message. A failed send releases the claim; a failure *recording* the hashes after a good send does not, since releasing then would send the same message twice. Titles only, as Telegram HTML with the three entities escaped, because titles carry names the user typed and a `_` breaks Markdown, whose plain-text fallback then drops the button. The one button opens `/analytics?tab=watchlist`.
...
compile-time guarantee: `quick-keyboard.test.ts` once asserted a fare resolved to Transportation
because the old `QUICK_FARES` was an `as const` list, and once tiles are user data the same property has to
...
existed. The user named these labels on this button, and a schedule guessing over a named one adds
spending to a label in `getLabelBreakdown`, which counts the full amount under every label a row carries.
