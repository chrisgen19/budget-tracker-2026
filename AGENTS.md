...
**Known gap:** nothing maps a Telegram account to an app user. `TELEGRAM_ALLOWED_IDS` says who may *talk* to the bot; `TELEGRAM_MCP_TOKEN` decides whose budget it writes to. So the prompt returns 409 rather than sending when more than one user has it enabled. A `users.telegram_user_id` column is what would lift that.
