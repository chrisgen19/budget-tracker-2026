...
- **[.claude/rules/receipts.md](.claude/rules/receipts.md)** — scanning and itemization, the scan-credit quota, receipt year repair, EXIF capture dates, and captions as hints. Loads on `src/lib/receipt-scan.ts`, `src/lib/receipt-date.ts`, `src/lib/receipt-guard.ts`, `src/lib/scan-quota.ts`, `src/lib/exif-date.ts`, `src/app/api/receipts`, `src/components/scan-receipt-sheet.tsx`, `src/components/multi-scan-review.tsx`, `src/components/scan-provider.tsx`.
- **[.claude/rules/gemini.md](.claude/rules/gemini.md)** — the model, fallback, thinking and timeout variables that govern every AI call. Loads on `src/lib/gemini.ts`, `src/lib/gemini-limits.ts`, `src/lib/receipt-scan.ts`, `src/app/api/receipts`, `src/lib/ai-assessment.ts`, `src/lib/telegram/classify.ts`.
