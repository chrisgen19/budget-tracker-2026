...
- **[.claude/rules/gemini.md](.claude/rules/gemini.md)** — the model, fallback, thinking and timeout variables that govern every AI call. Loads on `src/lib/gemini.ts`, `src/lib/receipt-scan.ts`, `src/lib/ai-assessment.ts`, `src/lib/telegram/classify.ts`.
