...
│   ├── account-balances.ts # Derived account balances (never stored)
│   ├── account-guard.ts    # Shared account-ownership check for the write paths
│   ├── transfer-filters.ts # Keeps TRANSFER rows out of spending aggregates
...
└── mcp-server/             # stdio entry point for the MCP server (local Claude Desktop)
    └── src/index.ts        # Thin wrapper; the 15 tools live in src/lib/mcp/server.ts
```
...
- **Accounts and transfers** (`src/lib/account-balances.ts`, `src/lib/transfer-filters.ts`) — an `Account` is a real place money sits: CASH, BANK, CREDIT_CARD or EWALLET. `Transaction.accountId` records which one a row moved through and is **nullable**, because every row written before accounts existed has no answer and inventing one is worse than admitting it; an unassigned row is simply outside every balance, and `openingBalance` is where the true starting figure goes. Balances are **derived, never stored**: rows here are edited, deleted, backdated and created days late by receipt scanning, so an incrementally maintained total drifts and nothing would notice. One signed convention throughout — positive is money you have, negative is money you owe — so a credit card is not a special case in the arithmetic, only in presentation, where `outstanding` is `-balance` (normalised so a paid-off card reads `0`, not `-0`). A `TRANSFER` moves money between two of the user's own accounts: `accountId` is the side it leaves, `transferAccountId` where it lands, and it is **not spending**, so it must never reach a category total. That is what removes the double-count from logging both a card purchase and the bill that settles it. TRANSFER is a third member of `TransactionType` rather than a separate table, which buys a guarantee from code that predates it: `Category.type` shares the enum and `createTransactionBatch` already enforces `category.type === transaction.type`, so the one Transfer system category cannot be used by an expense and no spending category by a transfer. That category is created by **migration `20260830100001`, not the seed**, since `pnpm db:seed` is not part of a deploy and the feature is unusable without it. The invariant is a database check constraint, not only a Zod refinement: a hand-written UPDATE or a future write path that forgot the rule would otherwise write money that leaves one account and arrives nowhere. Transfers are **never auto-labelled** — schedules classify spending by when it happened, and a card bill settled on a Tuesday afternoon would land in a weekday work window, where `getLabelBreakdown` splits half its amount into a spending label. The migration is split in two because PostgreSQL forbids *using* a new enum value in the transaction that adds it, and `prisma migrate deploy` runs each file in one
- **Nothing sums transactions with a binary ternary** (`src/lib/transfer-filters.ts`) — TypeScript cannot protect the code that gets transfers wrong. Nothing switches exhaustively on `TransactionType`, and `t.type === "INCOME" ? t.amount : -t.amount` compiles perfectly while filing every transfer as an expense. Adding one enum member made six sites wrong at once: the dashboard running balance and its balance-trend series, the analytics cash-flow buckets and its `ALL` category breakdown, `search_transactions`' per-category subtotals, the CSV export, and the analytics top-transactions list. All six route through `netDelta`, `isSpending` or `SPENDING_ONLY` now. Any new aggregate should either filter `type` explicitly to the direction it means or start from those, so the safe behaviour is the easy one. `asSpendingType` is the narrow exception, for boundaries where TRANSFER is structurally unreachable (a recurring bill, a user-created category) — use it only where you can name the reason
...
- `GET/POST /api/accounts` — list with derived balances (`?includeInactive`, `?asOf`, `?tz`) + create
- `PUT/DELETE /api/accounts/[id]` — update; `DELETE` archives an account that has transactions and deletes one that has none
- `GET/POST /api/categories` — list (defaults + custom) + create. Without an explicit `type`, the Transfer system category is excluded, so it never reaches a picker
