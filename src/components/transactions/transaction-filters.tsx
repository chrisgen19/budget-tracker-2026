...
export function TransactionFiltersBar({
  ...
  // Match ActionFab's mobile scroll behaviour: get the overlay out of the
  // reader's way immediately, then return it once scrolling has settled.
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolling(true);
  ...
  return (
    <>
      <section
        ...
        inert={isScrolling}
        className={cn(
          "card sticky top-[61px] lg:top-0 z-20 mb-4 overflow-hidden border-cream-300/70 bg-white shadow-soft",
          isScrolling
        ...
        <div className="p-2.5 sm:p-3">
          <div className="flex items-center gap-2.5">
            ...
            <MonthNavigator
              ...
              onSelectMonth={(month) => update({ month })}
              className="hidden sm:flex"
            />

            <TypeToggle filters={filters} onChange={update} className="hidden lg:flex" />

            <p aria-live="polite" className="hidden shrink-0 text-xs font-medium text-warm-400 xl:block">
              {totalCount === null
                ? "Loading…"
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-1.5">
              ...
              <select value={draftFilters.categoryId ?? ""} onChange={(event) => setDraftFilters((current) => ({ ...current, categoryId: event.target.value || null }))} className="min-h-11 w-full rounded-xl border border-cream-200 bg-cream-50/60 px-3 text-sm text-warm-700 outline-none transition focus:border-amber focus:bg-white focus:ring-2 focus:ring-amber/20">
            ...
            <label className="space-y-1.5">
              ...
              <select value={draftFilters.labelId ?? ""} onChange={(event) => setDraftFilters((current) => ({ ...current, labelId: event.target.value || null }))} disabled={labels.length === 0} className="min-h-11 w-full rounded-xl border border-cream-200 bg-cream-50/60 px-3 text-sm text-warm-700 outline-none transition focus:border-amber focus:bg-white focus:ring-2 focus:ring-amber/20 disabled:cursor-not-allowed disabled:text-warm-300">
                <option value="">{labels.length === 0 ? "No labels yet" : "All labels"}</option>
          ...
          <div className="sticky bottom-0 -mx-6 -mb-6 flex items-center gap-2 border-t border-cream-200 bg-white px-6 py-4">
            ...
            <button type="button" onClick={applyAdvancedFilters} disabled={amountRangeInvalid} className="min-h-11 flex-1 rounded-xl bg-amber px-5 text-sm font-semibold text-white shadow-soft transition-colors hover:bg-amber-dark disabled:cursor-not-allowed disabled:opacity-50">Apply filters</button>
          </div>
        </div>
      </Modal>
    </>
...
function MonthNavigator({
  ...
  return (
    <div className={cn("items-center justify-between rounded-xl border border-cream-200 bg-cream-50/60 p-0.5", className)}>
      ...
      <label className="relative flex min-h-10 min-w-0 flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg px-1 text-sm font-semibold text-warm-600 transition-colors hover:bg-white focus-within:bg-white focus-within:ring-2 focus-within:ring-amber/20 sm:min-w-32">
...
function AmountInput({
  ...
  return (
    <label className="relative flex-1">
      ...
      <input type="number" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} min="0" inputMode="decimal" aria-invalid={invalid} aria-describedby={describedBy} className="min-h-11 w-full rounded-xl border border-cream-200 bg-cream-50/60 py-2.5 pl-8 pr-3 text-sm text-warm-700 outline-none transition placeholder:text-warm-300 focus:border-amber focus:bg-white focus:ring-2 focus:ring-amber/20" />
    </label>
  );
}
