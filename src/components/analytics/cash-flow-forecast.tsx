...
export function CashFlowForecast({ timezoneOffset, currency, hideAmounts }: { timezoneOffset: number; currency: string; hideAmounts: boolean }) {
  ...
  // With cards on, the route rebases the forecast to cash, so the opening figure has to be what the
  // bank held. The old "tracked balance" wording invited bank-minus-card-debt instead, which the
  // route would then pay off a second time. See cards.md.
  const cash = useUser().user.creditCardsEnabled;
  ...
  return <div className="space-y-4">
    <div className="card p-5">
      ...
      <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-warm-600">Forecasts include future-dated transactions, active scheduled income and bills, credit card payments on each card&apos;s due day, plus daily flexible and savings budget pace. They cannot model transfers, unplanned spending, interest yet to be charged, or real account balances.</div>
    </div>
    {!data.configured ? <OpeningBalanceForm cash={cash} initialDate={data.today} maxDate={data.today} hideAmounts={hideAmounts} onDone={() => setEditing(false)} /> : <>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><Metric label={cash ? "Cash in the bank today" : "Tracked balance today"} value={fmt(data.trackedBalanceToday!)} detail={`From ${data.openingBalanceDate}`} /><Metric label="Lowest projected balance" value={data.lowestBalance ? fmt(data.lowestBalance.balance) : "—"} detail={data.lowestBalance ? data.lowestBalance.date : "No projection days"} /><Metric label="Cash crunches" value={String(data.cashCrunches?.length ?? 0)} detail={data.cashCrunches?.[0] ? `First: ${data.cashCrunches[0].date}` : "None projected"} /></div>
      ...
      <details className="card p-4 text-sm text-warm-500"><summary className="flex min-h-11 cursor-pointer items-center font-medium text-warm-700">Assumptions and uncertainty</summary><ul className="mt-3 list-disc space-y-1 pl-5">{data.assumptions.map((assumption) => <li key={assumption}>{assumption}</li>)}</ul></details>
      {editing && <OpeningBalanceForm cash={cash} initialDate={data.openingBalanceDate!} maxDate={data.today} initialBalance={data.openingBalance} hideAmounts={hideAmounts} onDone={() => setEditing(false)} />}
    </>}
  </div>;
}
...
function OpeningBalanceForm({ cash, initialDate, maxDate, initialBalance = 0, hideAmounts, onDone }: { cash: boolean; initialDate: string; maxDate: string; initialBalance?: number; hideAmounts: boolean; onDone: () => void }) {
  ...
  return <div className="card p-5"><div className="flex items-center gap-2"><Wallet className="h-5 w-5 text-amber-600" /><h3 className="font-serif text-lg text-warm-700">{cash ? "Set your bank balance" : "Set opening tracked balance"}</h3></div><p className="mt-1 text-sm text-warm-400">{cash ? "Enter what your bank account held at the start of this date, the figure your banking app shows. Don’t take off what you owe on cards: the forecast pays each card on its due date." : "Enter the balance at the start of this date. Logged activity after it is applied to calculate today’s tracked balance."}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm text-warm-600">Balance<input aria-label="Opening tracked balance" value={balance} onChange={(event) => setBalance(event.target.value)} type={hideAmounts ? "password" : "number"} inputMode="decimal" step="0.01" className="mt-1 min-h-11 w-full rounded-lg border border-cream-200 bg-white px-3" /></label><label className="text-sm text-warm-600">As of date<input aria-label="Opening balance date" value={date} onChange={(event) => setDate(event.target.value)} type="date" max={maxDate} className="mt-1 min-h-11 w-full rounded-lg border border-cream-200 bg-white px-3" /></label></div>{save.isError && <p className="mt-2 text-sm text-expense">Could not save this balance. Please try again.</p>}<div className="mt-4 flex gap-2"><button disabled={save.isPending || !date || !hasBalance || !Number.isFinite(Number(balance))} onClick={() => { if (hasBalance) save.mutate({ openingBalance: Number(balance), openingBalanceDate: date }, { onSuccess: onDone }); }} className="min-h-11 rounded-lg bg-amber-600 px-4 text-sm font-medium text-white disabled:opacity-50">{save.isPending ? "Saving…" : "Save forecast baseline"}</button></div></div>;
}
